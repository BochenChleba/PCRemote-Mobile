package com.example.pcremote.view.tablayout
...
class HighlightedTabLayout : TabLayout {
    ...
    private fun View.animateOnUnselection() =
        this.animate().scaleX(DEFAULT_SCALE).scaleY(DEFAULT_SCALE).alpha(
            TAB_ALPHA_FACTOR
        ).start()

    private fun View.animateOnSelection() =
        this.animate().scaleX(TAB_SCALE_FACTOR).scaleY(TAB_SCALE_FACTOR).alpha(
            DEFAULT_ALPHA
        ).start()
